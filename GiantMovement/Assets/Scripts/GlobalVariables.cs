    public static class GlobalVariables
{
    public const string CLOUDS = "Clouds"; 
    public const string PINCH_EXTENDER_LEFT = "PinchExtenderLeft";
    public const string PINCH_EXTENDER_RIGHT = "PinchExtenderRight";
    public const string PLAYER = "Player";
    public const string RAIN = "Rain";
    public const string RIVER = "River";
    public const string THUNDER_CLOUD = "ThunderCloud";
    public const string FINAL_RIVER = "FinalRiver";
    public const string GIANT = "Giant";
    public const string GIANT_WALK_ANIMATION = "Giant-04-walk";
    public const string GREEK_GOD = "LMHeadMountedRig1";
    public const string DUNGEON_MASTER = "DungeonMaster";
    public const string ANIM_BOOL_TAKE_DAMAGE = "TakeDamage";
    public const string ANIM_BOOL_GAME_OVER = "GameOver";
    public const string END_SCENE_NAME = "EndPage";
    public const string MAIN_SCENE_NAME = "Staging";
}
